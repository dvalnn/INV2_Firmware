...
/**************************************************************************/
/*!
    @brief  Sets the number of samples to average for temperature conversion.
    @param  samples Number of samples to average (1, 2, 4, 8, or 16). 
            Invalid values default to 1 sample (no averaging).
            CR1 register bits 6:4 control averaging:
            000 = 1 sample, 001 = 2 samples, 010 = 4 samples, 
            011 = 8 samples, 1xx = 16 samples
*/
/**************************************************************************/
void Adafruit_MAX31856::setAveragingSamples(uint8_t samples) {
  uint8_t t = readRegister8(MAX31856_CR1_REG);
  uint8_t avgBits = 0;  // Default to 000 (1 sample)
  
  // Determine the averaging bits based on input
  switch (samples) {
    case 2:
      avgBits = 0x10;  // 001 shifted to bits 6:4
      break;
    case 4:
      avgBits = 0x20;  // 010 shifted to bits 6:4
      break;
    case 8:
      avgBits = 0x30;  // 011 shifted to bits 6:4
      break;
    case 16:
      avgBits = 0x40;  // 100 shifted to bits 6:4 (1xx pattern)
      break;
    case 1:
    default:
      avgBits = 0x00;  // 000 = 1 sample (no averaging)
      break;
  }
  
  // Clear bits 6:4 and set new averaging bits
  t &= 0x8F;  // Clear bits 6:4 (mask: 10001111)
  t |= avgBits;  // Set new averaging bits
...
/**************************************************************************/
/*!
    @brief  Gets the current number of samples being averaged for temperature conversion.
    @returns Current number of samples (1, 2, 4, 8, or 16)
*/
/**************************************************************************/
uint8_t Adafruit_MAX31856::getAveragingSamples(void) {
  uint8_t t = readRegister8(MAX31856_CR1_REG);
  uint8_t avgBits = (t >> 4) & 0x07;  // Extract bits 6:4
  
  // Convert bit pattern to number of samples
  switch (avgBits) {
    case 0x01: return 2;   // 001
    case 0x02: return 4;   // 010
    case 0x03: return 8;   // 011
    case 0x04:             // 100 (16 samples)
    case 0x05:             // 101 (16 samples)
    case 0x06:             // 110 (16 samples)
    case 0x07: return 16;  // 111 (16 samples)
    case 0x00:             // 000
    default: return 1;     // 1 sample (no averaging)
  }
}
...
/**************************************************************************/
/*!
    @brief  set averaging time for the thermocouple voltage conversion averaging mode
*/
/**************************************************************************/
void setAveragingSamples(u_int8_t avg_samples){
